 ...
 */
struct TextColor {
  TextColor(Color xforeground = Color::none, Color xbackground = Color::none)
      : foreground(xforeground), background(xbackground) {}
...
struct Conditional {
  ...
  [[nodiscard]] virtual bool is_true() const { return func_(); }
...
struct Timestamp {
  ...
  Timestamp(const time_point_sys_clock& xtime_point)
      : time_point(xtime_point), is_null_(false) {}

  Timestamp(time_point_sys_clock&& xtime_point)
      : time_point(std::move(xtime_point)), is_null_(false) {}
 ...
 */
struct Function {
  Function(const std::string& xname, const std::string& xfile, size_t xline)
      : name(xname), file(xfile), line(xline), is_null_(false) {}

  Function(std::string&& xname, std::string&& xfile, size_t xline)
      : name(std::move(xname)),
        file(std::move(xfile)),
        line(xline),
        is_null_(false) {}
...
struct SinkFormat : public Sink {
  SinkFormat(const Filter& xfilter, const std::string& format)
      : Sink(xfilter), format_(format) {}
...
struct SinkFile : public SinkFormat {
  SinkFile(const Filter& xfilter, const std::string& filename,
           const std::string& format =
               "%Y-%m-%d %H-%M-%S.#ms [#severity] (#tag_func)")
      : SinkFormat(xfilter, format) {
...
struct SinkAndroid : public Sink {
  SinkAndroid(const std::string& ident, const Filter& xfilter)
      : Sink(xfilter), ident_(ident) {}
...
struct SinkEventLog : public Sink {
  SinkEventLog(const std::string& ident, const Filter& xfilter)
      : Sink(xfilter) {
#ifdef UNICODE
    std::wstring wide =
...
struct SinkNative : public Sink {
  SinkNative(const std::string& ident, const Filter& xfilter)
      : Sink(xfilter), log_sink_(nullptr), ident_(ident) {
#ifdef __ANDROID__
...
struct SinkCallback : public Sink {
  ...
  SinkCallback(const Filter& xfilter, callback_fun callback)
      : Sink(xfilter), callback_(callback) {}
